...
export class HttpExceptionFilter implements ExceptionFilter {
  ...
  constructor(options?: { logging: boolean }) {
    ...
    if (exception instanceof HttpException) {
      ...
      if (typeof exceptionResponse === 'string') {
        ...
        payload = this.handleHttpException(exceptionResponse, status, requestUrl)
      }
    } else {
      this.logger?.error(`[UnhandledException] ${requestUrl}`, exception)
    }
  ...
  private handleHttpException(
    ...
    this.logger?.error(`[HttpException] Status: ${status}, URL: ${requestUrl}`, exceptionResponse)
